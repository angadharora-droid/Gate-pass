...
export const db = {
  ...
  // in-memory at startup (see seedHashPasswords below) so no plaintext password
  // is ever stored on a user record or returned by the API.
  users: [
...
// Replace any plaintext seed passwords with salted scrypt hashes, one time, at
// startup. After this runs, every user record carries only `passwordHash`.
(function seedHashPasswords() {
...
let passCounter = Math.max(0, ...db.gatePasses.map(p => extractSequence(p.passNumber)));

export function generatePassNumber({ type, direction, returnable } = {}) {
  passCounter += 1;
  const year = new Date().getFullYear();
  const t = type === 'inward' ? 'I' : 'O';
  const d = direction === 'internal' ? 'I' : 'E';
  const r = returnable ? 'R' : 'N';
  return `GP-${t}${d}${r}-${year}-${String(passCounter).padStart(4, '0')}`;
}

export function logAudit(action, userId, targetId, details) {
  db.auditLog.push({
    id: uuidv4(),
    action,
    userId,
